...
# fraud_df['embedding'] = fraud_df['embedding'].apply(lambda x: [float(i) for i in x] if isinstance(x, list) else [])
fraud_docs = spark.createDataFrame(fraud_df)

fraud_docs.write.mode('overwrite').saveAsTable("workspace.llm.frauds_documentation_jacob")

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM workspace.llm.frauds_documentation_jacob LIMIT 10
...
VECTOR_SEARCH_ENDPOINT_NAME = 'test_jacob'
...
# MAGIC %sql
# MAGIC -- Enable Change Data Feed for the source table
# MAGIC ALTER TABLE workspace.llm.frauds_documentation_jacob SET TBLPROPERTIES (delta.enableChangeDataFeed = true);
...
source_table_fullname = f"{catalog}.{db}.frauds_documentation_jacob"
# Where we want to store our index
vs_index_fullname = f"{catalog}.{db}.frauds_documentation_index_jacob_6"
...
source_table_fullname = f"{catalog}.{db}.frauds_documentation_jacob"
# Where we want to store our index
vs_index_fullname = f"{catalog}.{db}.frauds_documentation_index_jacob_3"
...
# VECTOR_SEARCH_ENDPOINT_NAME = 'test_jacob'
# vs_index_fullname = 'workspace.llm.frauds_documentation_index_jacob_6'
...
vs_index_fullname = "workspace.default.vs_index"
